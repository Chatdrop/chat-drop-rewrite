...
const recursive_delete = async (dir_path) => {
  try {
    const items = await fs.readDir(dir_path);
    
    // Process all items in parallel
    await Promise.all(items.map(async (item) => {
      if (item.isDirectory()) {
        // Recursively delete directory contents, then the directory itself
        await recursive_delete(item.path);
    ...
    // Directory might not exist or already be empty, that's ok
    console.log(`Error deleting ${dir_path}:`, error);
  }
};

const reset_persistance = async () => {
  try {
    // Torch everything in both directories
    await Promise.all([
      recursive_delete(persist_dir),
      recursive_delete(fs.CachesDirectoryPath)
    ]);
  
    
    BackHandler.exitApp();
    
    console.log("Successfully torched all persistence and cache data");
  } catch (error) {
